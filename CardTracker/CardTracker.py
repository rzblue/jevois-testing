...
class CardTracker:
    ...
    def process(self, inframe, outframe):
        ...
        inFrameBGR = inframe.getCvBGR()

        # Start measuring image processing time (NOTE: does not account for input conversion time):
        self.timer.start()

        # Run the pipeline
        self.pipeline.process(inFrameBGR)
        #index = 0
        #for contour in self.pipeline.find_contours_output:
        #    if index >= 3:
        #        break
        #    rect = cv2.minAreaRect(contour)
        #    box = cv2.boxPoints(rect)
        #    box = np.int0(box)
        #    cv2.drawContours(inFrameBGR,[box],0,(0,0,255),2)
        #    index++
        # Write frames/s info from our timer into the edge map (NOTE: does not account for output conversion time):
        fps = self.timer.stop()
        cv2.putText(inFrameBGR, fps, (0,10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0), 1, cv2.LINE_AA)

        # Convert our GRAY output image to video output format and send to host over USB:
        outframe.sendCvBGR(inFrameBGR)
